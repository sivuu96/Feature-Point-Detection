...
float GoICP::OuterBnB()
{
	...
	{       z++;
        ...
        /*ofstream ofile_err;
        ofile_err.open("arm1_err_out0.txt",std::ios_base::app);
        ofile_err<<File_c<<"\t"<<error<<endl;
	ofile_err.close();
	string File_str= to_string(File_c);
        
        string output_File="arm1_output"+File_str+".txt";
         */
	ofstream ofile;
          
	ofile.open("feature.txt", ofstream::out);
	for(i = 0; i < Nd; i++)
	{		
		ofile<< pDataTempICP[i].x<<" "<<pDataTempICP[i].y<<" "<<pDataTempICP[i].z<< endl;
